...
class connect
{
    ...
    /**
     * @param $columb  столбцы
     * @param $first_table  таблица
     * @param $where  id ля поиска
     */
    public function getDataById($columb, $first_table, $where)
    {
        $columb = (string)$columb;
        $first_table = (string)$first_table;
        $where = (int)$where;

        $query = "SELECT " . $columb . " FROM " . $first_table . " ";

        $query .= "WHERE " . $first_table . "_id=:id";

        $data = $this->db->prepare($query);
        $data->bindValue(':id', $where, PDO::PARAM_INT);
        $data->execute();
